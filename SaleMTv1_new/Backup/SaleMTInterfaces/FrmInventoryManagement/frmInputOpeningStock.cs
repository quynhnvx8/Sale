...
{
    public partial class frmInputOpeningStock : Form
    {
        #region Declaration
        ...
        #region Contructor
        public frmInputOpeningStock(Dictionary<string, string> language)
        {
            translate = language;
            InitializeComponent();
            initLanguage();
        ...
        {
            this.groupBox1.Text = translate["frmInputOpeningStock.groupBox1.Text"];
            this.btnImportExcel.Text = translate["frmInputOpeningStock.btnImportExcel.Text"];
            this.btnBegin.Text = translate["frmInputOpeningStock.btnBegin.Text"];
            this.label2.Text = translate["frmInputOpeningStock.label2.Text"];
            this.label1.Text = translate["frmInputOpeningStock.label1.Text"];
            this.groupBox2.Text = translate["frmInputOpeningStock.groupBox2.Text"];
            this.ProductCode.HeaderText = translate["frmInputOpeningStock.ProductCode.HeaderText"];
            this.Quantity.HeaderText = translate["frmInputOpeningStock.Quantity.HeaderText"];
            this.lblStatus.Text = translate["frmInputOpeningStock.lblStatus.Text"];
            this.btnClose.Text = translate["frmInputOpeningStock.btnClose.Text"];
            this.btnOK.Text = translate["frmInputOpeningStock.btnOK.Text"];
            this.lblSum.Text = translate["frmInputOpeningStock.lblSum.Text"];
            this.Text = translate["frmInputOpeningStock.Text"];
        }	
                ...
                if (dgvInventory.DataSource != null && dgvInventory.RowCount > 0)
                {
                    ...
                    foreach (DataGridViewRow row in dgvInventory.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            p.ProductID = row.Cells["ProductCode"].Value.ToString();
                            p.ProductQuantity = int.Parse(row.Cells["Quantity"].Value.ToString());
                            p.CreatedDate = dtpImportDate.Value;
                            okList.Add(p);
                        }
                    }
        ...
        private void ResetForm()
        {
            try
            {
                ...
                pbrProcess.Value = 0;
                cboSheet.Items.Clear();
                dtpImportDate.Value = sqlDac.GetDateTimeServer();
                dgvInventory.DataSource = null;
            }
            catch (Exception ex)
            {
        ...
        private void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            //dr1["ProductCode"] = "02aa12";
            //dr1["Quantity"] = 1000;
            //dt.Rows.Add(dr1);
            //dr2["ProductCode"] = "02aa22";
            //dr2["Quantity"] = 1000;
            //dt.Rows.Add(dr2);

            ExportExcel ex = new ExportExcel();
            ex.ExportSample(dt, "LIST_INVENTORY_PRODUCT_IMPORT.xls");

            //SaleMTCommon.Export expExcel = new SaleMTCommon.Export();
            //expExcel.InfoCompany = false;
            //expExcel.InfoStore = false;
            //expExcel.StrTitle = "";
            //expExcel.ExportSampleAndData(dt, "LIST_INVENTORY_PRODUCT_IMPORT.xls");
        }
    }
}
