    ...
    public class posdb_vnmSqlDAC  //StandardDAC.SqlClient.SQLDac  posdb_vnmSqlDAC.save
    {
        ...
        public static void SaveSync(object obj, string table, bool isNew)
        {
            try
            {
                ...
                using (NpgsqlConnection sqlConnection = new NpgsqlConnection(connSync))
                {
        ...
        public static string getListKey(string tableName, string keyColumn, bool isString, string whereClause)
        {
            string keySelect = "string_agg(cast(" + keyColumn + " as NVARCHAR(MAX)), ',')";
            if (isString)
                keySelect = "''''+ string_agg(cast(" + keyColumn + " as NVARCHAR(MAX)), ''',''') + ''''";
            string sql = "Select "+ keySelect +" as Keys From " + tableName + "";
        ...
        public static DataTable SelectData_Npgsql(string query, string[] paramName, object[] paramValue)
        {
            NpgsqlConnection sqlConnection = new NpgsqlConnection(connSync);
            sqlConnection.Open();
        ...
        public void PutData_Npgsql(string sqlString, NpgsqlParameter[] parameters)
        {
        ...
        #endregion synchonized EONE ADMIN
