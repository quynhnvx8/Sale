    ...
    public class ExportProcess
    {
        ...
        public static DataTable GetInventoryTemp()
        {
            ...
            try
            {
                ...
                //string query = "select INVENTORY_TEMP.PRODUCT_ID as ProductId,SUM(AMOUNT) as Quantity, PRODUCT_NAME as ProductName, PRODUCTS.UNIT_NAME as Unit, (PRODUCTS.PRICE * 1.1) as Price from INVENTORY_TEMP join PRODUCTS on INVENTORY_TEMP.PRODUCT_ID = PRODUCTS.PRODUCT_ID where STORE_CODE = @STORE_CODE group by INVENTORY_TEMP.PRODUCT_ID , PRODUCT_NAME, PRODUCTS.UNIT_NAME,PRODUCTS.PRICE having SUM(AMOUNT) <> 0 order by INVENTORY_TEMP.PRODUCT_ID";
                SqlParameter[] sqlPara = new SqlParameter[1] { posdb_vnmSqlDAC.newInParam("@STORE_CODE", SqlDbType.VarChar, storeCode) };
                dt = SqlDAC.GetDataTable("INVENTORY_TEMP_TK", sqlPara);
            }
            catch (Exception ex)
            {
        ...
        public static void DeleteInvoice(string storeid)
        {
            try
            {
                ...
                MessageBox.Show(rsfExportManagement.Export7.ToString(), "Notice", MessageBoxButtons.OK,
