...
namespace SaleMTInterfaces.FrmInventoryManagement
{
    public partial class frmOrder : TabForm
    {
        ...
        #region QUYNHNV.X8: Add 
        int client_ID = AppConfigFileSettings.Client_ID;
        private List<PO_FORECAST> listForeCast = new List<PO_FORECAST>();
        private List<PO_DVKH> listDVKH;
        private List<PO_SAFETY_STOCK> listSafety;
        private List<PO_SAFETY_STOCK_DETAIL> listSafetyDetail = new List<PO_SAFETY_STOCK_DETAIL>();
        private List<PO_SALES_DAY> listSalesDay = new List<PO_SALES_DAY>();

        private void getForecast()
        {
            string sql = "Select l.Qty, f.AD_Department_ID,  " +
                "       (Select b.Value From AD_Department b Where b.AD_Department_ID = f.AD_Department_ID) DeptCode, " +
                "       (Select Value From M_Product p Where l.M_Product_ID = p.M_Product_ID) ProductCode, " +
                "       (Select FiscalYear From C_Year y Where f.C_Year_ID = y.C_Year_ID) Years, " +
                "       (Select PeriodNo From C_Period p Where l.C_Period_ID = p.C_Period_ID) Months " +
                " From M_Forecast f Inner Join M_ForecastLine l on f.M_Forecast_ID = l.M_Forecast_ID " +
                " Where f.AD_Client_ID = " + client_ID + " And f.DocStatus = 'CO' And f.Updated >= current_date - 10";
            ...
            if(dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    if (row["AD_Department_ID"].ToString().Equals(UserImformation.DeptCode))
                    {
                        PO_FORECAST poForeCast = new PO_FORECAST();
                        poForeCast.PRODUCT_ID = row["ProductCode"].ToString();
                        poForeCast.QUANTITY = int.Parse(row["Qty"].ToString());
                        poForeCast.STORE_CODE = row["DeptCode"].ToString();
                        poForeCast.YEAR = int.Parse(row["Years"].ToString());
                        poForeCast.MONTH = int.Parse(row["Months"].ToString());
                        listForeCast.Add(poForeCast);
                    }
                }
            }            
        ...
        private void getSaleDay()
        {
            string sql = "Select l.DayOfMonth,  " +
                "       (Select b.Value From AD_Department b Where b.AD_Department_ID = f.AD_Department_ID) DeptCode, " +
                "       (Select FiscalYear From C_Year y Where f.C_Year_ID = y.C_Year_ID) Years, " +
                "       (Select PeriodNo From C_Period p Where l.C_Period_ID = p.C_Period_ID) Months" +
                " From C_SaleDay f Inner Join C_SaleDayLine l on f.C_SaleDay_ID = l.C_SaleDay_ID " +
                " Where f.AD_Client_ID = " + client_ID + " And f.DocStatus = 'CO' And f.Updated >= current_date - 10";
            DataTable dt = posdb_vnmSqlDAC.SelectData_Npgsql(sql, null, null);
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    PO_SALES_DAY poSalesDay = new PO_SALES_DAY();
                    poSalesDay.STORE_CODE = row["DeptCode"].ToString();
                    poSalesDay.YEAR = int.Parse(row["Years"].ToString());
                    poSalesDay.MONTH = int.Parse(row["Months"].ToString()); ;
                    poSalesDay.SALES_DAY = int.Parse(row["DayOfMonth"].ToString()); ;
                    listSalesDay.Add(poSalesDay);
                }
            }
        }

        private void getSafetyStock()
        {
            string sql = "Select l.MinQty, l.Category, l.DeliveryDays, l.Percent,  " +
                "       (Select b.Value From AD_Department b Where b.AD_Department_ID = f.AD_Department_ID) DeptCode, " +
                " From M_SafetyStock f Inner Join M_SafetyStockLine l on f.M_SafetyStock_ID = l.M_SafetyStock_ID " +
                " Where f.AD_Client_ID = " + client_ID + " And f.DocStatus = 'CO' And f.Updated >= current_date - 10";
            DataTable dt = posdb_vnmSqlDAC.SelectData_Npgsql(sql, null, null);
            if (dt.Rows.Count > 0)
            {
                listSafety = new List<PO_SAFETY_STOCK>();
                foreach (DataRow row in dt.Rows)
                {
                    PO_SAFETY_STOCK poSafety = new PO_SAFETY_STOCK();
                    poSafety.STORE_CODE = row["DeptCode"].ToString();
                    poSafety.CAT = row["Category"].ToString();
                    poSafety.MINSF = int.Parse(row["MinQty"].ToString());
                    poSafety.LEAD = int.Parse(row["DeliveryDays"].ToString());
                    poSafety.CALENDAR_D = row["DeliveryDays"].ToString();
                    poSafety.PERCENTAGE = int.Parse(row["Percent"].ToString());
                    listSafety.Add(poSafety);
                }
            }
        }

        private void getSafetyStockDetail()
        {
            string sql = "Select l.MinQty, l.Category, l.DeliveryDays, l.Percent, l.NextDelivery,  " +
                "       (Select Value From M_Product p Where l.M_Product_ID = p.M_Product_ID) ProductCode, " +
                "       (Select b.Value From AD_Department b Where b.AD_Department_ID = f.AD_Department_ID) DeptCode " +
                " From M_SafetyStock f Inner Join M_SafetyStockLine l on f.M_SafetyStock_ID = l.M_SafetyStock_ID " +
                " Where f.AD_Client_ID = " + client_ID + " And f.DocStatus = 'CO' And f.Updated >= current_date - 10";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    PO_SAFETY_STOCK_DETAIL poSafetyDetail = new PO_SAFETY_STOCK_DETAIL();
                    poSafetyDetail.STORE_CODE = row["DeptCode"].ToString();
                    poSafetyDetail.PRODUCT_ID = row["ProductCode"].ToString();
                    poSafetyDetail.CAT = row["Category"].ToString();
                    poSafetyDetail.MINSF = int.Parse(row["MinQty"].ToString());
                    poSafetyDetail.LEAD = int.Parse(row["DeliveryDays"].ToString());
                    poSafetyDetail.CALENDAR_D = row["NextDelivery"].ToString();
                    poSafetyDetail.PERCENTAGE = int.Parse(row["Percent"].ToString());
                    listSafetyDetail.Add(poSafetyDetail);
                }
            }
        }
        ...
        private void getSO()
        {
            string sql = "Select AD_User_ID, Value, Name, Password, Created " +
                " From AD_User " +
                " Where AD_Client_ID = " + client_ID + " And IsActive = 'Y' And Updated >= current_date - 10";
            DataTable dt = posdb_vnmSqlDAC.SelectData_Npgsql(sql, null, null);
            if (dt.Rows.Count > 0)
            {
                listDVKH = new List<PO_DVKH>();
                foreach (DataRow row in dt.Rows)
                {
                    PO_DVKH poDVKH = new PO_DVKH();
                    poDVKH.DistCode = row["DistCode"].ToString();
        ...
        private void getPO()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow rowDetail in dt.Rows)
                {
                    string[] split = Regex.Split(rowDetail["POCONumber"].ToString(), "_");
                    PO_DVKH poDVKH = new PO_DVKH();
                    poDVKH.DistCode = rowDetail["DistCode"].ToString();
                    ...
                    poDVKH.ItemDescr = rowDetail["ItemDescr"].ToString();
                    poDVKH.LineTotal = float.Parse(rowDetail["LineTotal"].ToString());
                    poDVKH.OrderDate = DateTime.Parse(rowDetail["OrderDate"].ToString());
                    poDVKH.POCONumber = split[0];
                    poDVKH.PONumber = rowDetail["PONumber"].ToString();
                    poDVKH.Price = decimal.Parse(rowDetail["Price"].ToString());
                    poDVKH.Quantity = float.Parse(rowDetail["Quantity"].ToString());
        ...
        private void ImportFrom_Npgsql()
        {

            getForecast();
            getSaleDay();
            //getSafetyStock();//Bo cai nay
            getSafetyStockDetail();
            //getSO();
            //getPO();
